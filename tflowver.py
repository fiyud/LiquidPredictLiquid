...
from tensorflow.keras.layers import Dense, Conv2D, MaxPooling2D, Flatten, Dropout, GRU, Input, concatenate, LSTM
...
# Cell 13: Build GRU models
def build_proposed_gru_model(sequence_length, feature_dim):
    model = Sequential([
        GRU(128, return_sequences=True, input_shape=(sequence_length, feature_dim)),
        Dropout(0.2),
...
def build_baseline_gru_model(sequence_length, feature_dim):
    model = Sequential([
        GRU(64, return_sequences=True, input_shape=(sequence_length, feature_dim)),
        Dropout(0.2),
...
proposed_gru_model = build_proposed_gru_model(X_seq.shape[1], X_seq.shape[2])
print("Proposed GRU model built")
print(f"Input shape: {X_seq.shape}")

baseline_ts_sequences, _ = create_sequences_for_gru(ts_features, labels, time_steps=4)
baseline_gru_model = build_baseline_gru_model(baseline_ts_sequences.shape[1], baseline_ts_sequences.shape[2])
print("Baseline GRU model built")
print(f"Baseline input shape: {baseline_ts_sequences.shape}")

# Cell 14: Train GRU models
scaler_y = MinMaxScaler()
y_seq_scaled = scaler_y.fit_transform(y_seq.reshape(-1, 1)).flatten()

X_train = X_seq[:train_size]
y_train_scaled = y_seq_scaled[:train_size]
X_val = X_seq[train_size:train_size+val_size]
y_val_scaled = y_seq_scaled[train_size:train_size+val_size]
X_test = X_seq[train_size+val_size:]
y_test_scaled = y_seq_scaled[train_size+val_size:]

print("Training proposed GRU model...")
history_proposed = proposed_gru_model.fit(
...
print("Training baseline GRU model...")
history_baseline = baseline_gru_model.fit(
    baseline_X_train, y_train_scaled,
    validation_data=(baseline_X_val, y_val_scaled),
    epochs=50,
    batch_size=32,
    verbose=1
)
...
y_pred_proposed_scaled = proposed_gru_model.predict(X_test, verbose=1)
y_pred_baseline_scaled = baseline_gru_model.predict(baseline_X_test, verbose=1)

y_pred_proposed = scaler_y.inverse_transform(y_pred_proposed_scaled.reshape(-1, 1)).flatten()
...
plt.subplot(2, 2, 1)
plt.plot(history_proposed.history['loss'], label='Proposed Training Loss')
plt.plot(history_proposed.history['val_loss'], label='Proposed Validation Loss')
...
# Cell 20: Save results and additional analysis
