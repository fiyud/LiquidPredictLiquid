...
def physics_loss(predicted_levels, inflow, outflow, area=1000.0, dt=1.0):
    if len(predicted_levels) <= 1:
        return torch.tensor(0.0, device=predicted_levels.device)
    
    # Water balance: dV/dt = Inflow - Outflow
    # V = Area × Height
    predicted_volume_change = area * (predicted_levels[1:] - predicted_levels[:-1]) / dt
    actual_volume_change = inflow[:-1] - outflow[:-1]
    
    # Ensure same length
    min_len = min(len(predicted_volume_change), len(actual_volume_change))
    predicted_volume_change = predicted_volume_change[:min_len]
    actual_volume_change = actual_volume_change[:min_len]
    
    return nn.MSELoss()(predicted_volume_change, actual_volume_change)
...
# Cell 13: Memory-efficient Dataset class
class WaterLevelDataset(Dataset):
    def __init__(self, sequences, targets, scaler_y=None, fit_scaler=False):
        # Store as float32 to save memory
        self.sequences = torch.FloatTensor(sequences).half()  # Use half precision
        ...
        self.targets = torch.FloatTensor(targets_scaled).half()  # Use half precision
        
    def __len__(self):
        return len(self.sequences)
    
    def __getitem__(self, idx):
        return self.sequences[idx].float(), self.targets[idx].float()  # Convert back to float32
...
# Cell 15: Enhanced training function with physics loss
def train_model_with_physics(model, train_loader, val_loader, num_epochs=50, lr=0.001, physics_weight=0.1):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (sequences, targets) in enumerate(train_loader):
            ...
            # Physics loss (water balance constraint)
            if batch_idx < len(train_loader) - 1:  # Skip last batch if size mismatch
                # Get inflow and outflow data for physics loss
                batch_size = sequences.shape[0]
                start_idx = batch_idx * batch_size
                end_idx = min(start_idx + batch_size, len(df) - 1)
                
                if end_idx > start_idx:
                    inflow_batch = torch.FloatTensor(df['Inflow_m3s'].values[start_idx:end_idx]).to(device)
                    outflow_batch = torch.FloatTensor(df['TotalDischarge_m3s'].values[start_idx:end_idx]).to(device)
                    
                    if len(outputs) > 1:
                        phys_loss = physics_loss(outputs, inflow_batch[:len(outputs)], 
                                               outflow_batch[:len(outputs)], area=1000.0, dt=1.0)
                        physics_loss_total += phys_loss.item()
                    else:
                        phys_loss = 0
                else:
                    phys_loss = 0
            ...
            # Combined loss
            total_loss = mse_loss + physics_weight * phys_loss if isinstance(phys_loss, torch.Tensor) else mse_loss
            total_loss.backward()
            optimizer.step()
        ...
        physics_loss_avg = physics_loss_total / len(train_loader)
        
        train_losses.append(train_loss)
        val_losses.append(val_loss)
        physics_losses.append(physics_loss_avg)
        
        if (epoch + 1) % 10 == 0:
            print(f'Epoch [{epoch+1}/{num_epochs}], Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, Physics Loss: {physics_loss_avg:.4f}')
    
    return train_losses, val_losses, physics_losses

print("Training proposed model with physics constraints...")
proposed_train_losses, proposed_val_losses, proposed_physics_losses = train_model_with_physics(
    proposed_model, train_loader, val_loader, physics_weight=0.1)

print("Training baseline model with physics constraints...")
baseline_train_losses, baseline_val_losses, baseline_physics_losses = train_model_with_physics(
    baseline_model, baseline_train_loader, baseline_val_loader, physics_weight=0.1)
...
plt.subplot(2, 3, 3)
plt.plot(proposed_physics_losses, label='Proposed Physics Loss', color='red')
plt.plot(baseline_physics_losses, label='Baseline Physics Loss', color='orange')
plt.title('Physics Loss During Training')
plt.xlabel('Epoch')
