...
from LiquidMamba import LiquidMambaAttention, 
...
# Cell 6: Define UNet feature extractor with GeM and Multi-Scale features
class GeM(nn.Module):
...
class UNetMSFeatureExtractor(nn.Module):
    """
    UNet-based Multi-Scale Feature Extractor
    Rút đặc trưng từ các tầng UNet encoder: x3, x4, x5 (multi-scale) -> concat -> head -> 1024-d.
    - Sử dụng UNet encoder để trích xuất đặc trưng đa cấp độ
    - Đóng băng backbone để ổn định khi chỉ train LiquidMamba
    - Dùng GeM (ổn định hơn GAP) + LayerNorm + MLP nhỏ
    """
    def __init__(self, input_channels=1, output_features=1024, freeze_backbone=True):
        super().__init__()
        
        # Initialize UNet model for feature extraction
        self.unet = UNet(
            n_channels=input_channels, 
            n_classes=2,  # dummy classes for segmentation head
            bilinear=False,
            c3str_layers=2,
            c3str_heads=8,
            c3str_mlp_ratio=4,
        ...
        # Calculate feature dimensions from UNet encoder stages
        # Based on UNet architecture: x3 (256), x4 (512), x5 (1024)
        # After down1 (128), down2 (256), down3 (512), down4 (1024)
        with torch.no_grad():
            dummy_input = torch.zeros(1, input_channels, 320, 320)
            x1 = self.unet.inc(dummy_input)    # 64 channels
            x2 = self.unet.down1(x1)           # 128 channels  
            x3 = self.unet.down2(x2)           # 256 channels
            x4 = self.unet.down3(x3)           # 512 channels
            x5 = self.unet.down4(x4)           # 1024 channels (with C3STR)
            
            # Get actual channel dimensions
            c3, c4, c5 = x3.shape[1], x4.shape[1], x5.shape[1]
            concat_dim = c3 + c4 + c5
        
        print(f"UNet feature dimensions - x3: {c3}, x4: {c4}, x5: {c5}, total: {concat_dim}")
        ...
        if freeze_backbone:
            # Freeze UNet encoder parameters
            for p in self.unet.parameters():
                p.requires_grad = False
            # Only allow training of head and norm
            for p in self.head.parameters():
                p.requires_grad = True
            for p in self.norm.parameters():
                p.requires_grad = True
        
        self.eval()  # Set to eval mode for stable feature extraction

    @torch.no_grad()
    def forward(self, x):
        """
        Extract multi-scale features using UNet encoder
        """
        # UNet encoder forward pass
        x1 = self.unet.inc(x)       # Initial conv: 64 channels
        x2 = self.unet.down1(x1)    # Down 1: 128 channels
        x3 = self.unet.down2(x2)    # Down 2: 256 channels, ~80x80
        x4 = self.unet.down3(x3)    # Down 3: 512 channels, ~40x40
        x5 = self.unet.down4(x4)    # Down 4: 1024 channels with C3STR, ~20x20
        
        # Apply GeM pooling to multi-scale features
        z3 = self.pool(x3)  # (B, 256)
        z4 = self.pool(x4)  # (B, 512)  
        z5 = self.pool(x5)  # (B, 1024)
        
        # Concatenate multi-scale features
        z = torch.cat([z3, z4, z5], dim=1)  # (B, concat_dim)
        z = self.norm(z)
        z = self.head(z)  # (B, output_features)
        return z

# khởi tạo UNet extractor (đóng băng backbone, chỉ rút đặc trưng)
unet_model = UNetMSFeatureExtractor(input_channels=1, output_features=1024, freeze_backbone=True).to(device)
...
print(f"\nPROPOSED MODEL (UNet + MultiScale LiquidMamba) RESULTS:")
...
torch.save(proposed_model.state_dict(), 'proposed_unet_liquidmamba_model.pth')
torch.save(baseline_model.state_dict(), 'baseline_liquidmamba_model.pth')
print("UNet + LiquidMamba models saved successfully!")