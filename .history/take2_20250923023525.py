...
# Cell 10: Define Cross-Modal Attention and Physics Loss
class CrossModalAttention(nn.Module):
    def __init__(self, image_dim, ts_dim, hidden_dim):
        super().__init__()
    ...
    def forward(self, image_features, ts_features):
        # Cross-attention between image and time series features
...
# Combine multimodal features using cross-modal attention
...
# Use attention for feature fusion
...
img_tensor = torch.FloatTensor(image_features_norm).to(device)
ts_tensor = torch.FloatTensor(ts_features_norm).to(device)

with torch.no_grad():
    attended_features = attention_module(img_tensor, ts_tensor)
    combined_features = attended_features.cpu().numpy()
